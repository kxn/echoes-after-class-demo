...
JOB=ROOT/'assets/animation/npc-exit-v1'
def main():
    ...
    contact.save(ROOT/'docs/broadcast/normalized-contact.jpg')
    ...
    atlas(selected,ROOT/'assets/relit/npc_2_walk_albedo.png')
    atlas([normal_map(f) for f in selected],ROOT/'assets/relit/npc_2_walk_normal.png')
    contacts=[int(x) for x in args.contacts.split(',') if x]
    info=dict(source='npc-exit-v1/video.webm',rgba_directory='assets/animation/npc-exit-v1/rgba',source_frames=56,start=args.start,end_exclusive=args.end,frames=len(selected),fps=24,columns=8,cell_width=384,cell_height=640,foot_y=610,contact_frames=contacts,stride_world_units=1.9,transform=dict(scale=scale,dx=dx,dy=dy),stabilization='one shared subpixel transform; no framewise anchor snapping',loop_candidates=candidates[:8])
    (ROOT/'assets/relit/npc_2_walk.json').write_text(json.dumps(info,indent=2))
    preview=[]
    for i,f in enumerate(selected):
        ...
        preview.append(bg)
    preview[0].save(ROOT/'docs/broadcast/walk-loop.gif',save_all=True,append_images=preview[1:],duration=42,loop=0)
